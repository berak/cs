...
HRESULT FGraph::displayPinProperties( IPin * pSrcPin, HWND hWnd)
{
...
IBaseFilter *FGraph::getFirstDevice( const GUID &cat, const WCHAR *name ) 
{
    ...
    if ( hr == S_OK ) {
        while (pClassEnum->Next(1, &pMoniker, &cFetched) == S_OK){
            ...
            #ifdef TL_DEBUG
                tl_trace(_lf,__FILE__,__LINE__,"TL_MO_BIND pSrc %p\n",pSrc);
            #endif
            pMoniker->Release();
            if ( pSrc && hr==S_OK ) {
...
HRESULT FGraph::writeBitmap( TCHAR *bmpName, BYTE *pBuffer, long len, int w, int h )
{
